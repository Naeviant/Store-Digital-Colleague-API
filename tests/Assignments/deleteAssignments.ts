...
export const deleteAssignments = async (token: string) => {
	return new Promise<void>(async (resolve, reject) => {
		...
		await axios.delete(`${config.base}/assignment/A/1/2/Multi-Location/0/`, headers).then(() => {
		...
		await axios.delete(`${config.base}/assignment/-1/A/2/Multi-Location/0/`, headers).then(() => {
		...
		await axios.delete(`${config.base}/assignment/-1/1/2/Invalid/0/`, headers).then(() => {
