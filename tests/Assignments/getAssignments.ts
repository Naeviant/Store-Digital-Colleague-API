...
export const getAssignments = async (token: string) => {
	return new Promise<void>(async (resolve, reject) => {
		...
		await axios.get(`${config.base}/assignment/location/A/1/1`, headers).then(() => {
		...
		await axios.get(`${config.base}/assignment/location/-1/A/1`, headers).then(() => {
		...
		await axios.get(`${config.base}/assignment/location/-1/1/A`, headers).then(() => {
