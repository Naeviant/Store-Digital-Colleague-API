...
export const getAisle = async (req: Request & { params: { aisle: number } }, res: Response): Promise<void> => {
	try {
		if (!req.params.code || !req.params.aisle) res.status(400).send(new SiteResponse(400, 'Cannot Get Aisle: Invalid Site Code or Aisle Number Provided'));
		else axios.get(`http://localhost:8000/api/site/${req.params.code}`).then((response: AxiosResponse) => {
			const site = response.data.data;
...
export const updateAisle = async (req: Request & { params: { aisle: number } }, res: Response): Promise<void> => {
	try {
		if (!req.params.code || !req.params.aisle) res.status(400).send(new SiteResponse(400, 'Cannot Update Aisle: Invalid Site Code or Aisle Number Provided'));
		else axios.get(`http://localhost:8000/api/site/${req.params.code}`).then((response: AxiosResponse) => {
			const site = response.data.data;
...
export const deleteAisle = async (req: Request & { params: { aisle: number } }, res: Response): Promise<void> => {
	try {
		if (!req.params.code || !req.params.aisle) res.status(400).send(new SiteResponse(400, 'Cannot Delete Aisle: Invalid Site Code or Aisle Number Provided'));
		else axios.get(`http://localhost:8000/api/site/${req.params.code}`).then((response: AxiosResponse) => {
			const site = response.data.data;
			Aisle.deleteOne({ site: site._id, aisle: req.params.aisle }).then((doc: { deletedCount: number }) => {
				if (doc.deletedCount === 0) res.status(400).send(new SiteResponse(400, 'Cannot Delete Aisle: Invalid Site Code or Aisle Number Provided'));
