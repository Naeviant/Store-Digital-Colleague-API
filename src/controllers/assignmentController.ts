...
export const addAssignment = async (req: Request, res: Response): Promise<void> => {
	try {
		if (!req.params.code || !req.params.aisle || !req.params.bay) respond(req, res, 400, 'Cannot Assign Product: Invalid Site Code, Aisle Number or Bay Number Provided');
		else if (!req.body.ean || !req.body.type) respond(req, res, 400, 'Cannot Assign Product: Invalid Request Body');
			...
			if (
				...
				newAssignment.save().then(() => {
					...
					if (error) respond(req, res, 400, 'Cannot Assign Product: Invalid Request Body');
				});
			}).catch((error: Error & { response: { status: number } } | null) => {
				if (error && error.response && (error.response.status === 404 || error.response.status === 400)) respond(req, res, 400, 'Cannot Assign Product: Invalid EAN Provided');
				else if (error) generate500(req, res, error);
			}); 
		}).catch((error: Error & { response: { status: number } } | null) => {
			if (error && error.response && (error.response.status === 404 || error.response.status === 400)) respond(req, res, 400, 'Cannot Assign Product: Invalid Site Code, Aisle Number or Bay Number Provided');
			else if (error) generate500(req, res, error);
		}); 
	} catch (error) {
...
export const getAssignmentsByLocation = async (req: Request, res: Response): Promise<void> => {
	try {
		if (!req.params.code || !req.params.aisle || !req.params.bay || !req.params.type) respond(req, res, 400, 'Cannot Get Assignments: Invalid Site Code, Aisle Number, Bay Number or Assignment Type Provided');
		else axios.get(`${config.base}/bay/${req.params.code}/${req.params.aisle}/${req.params.bay}`).then((response: AxiosResponse) => {
			const bay = response.data.data;
...
export const getAssignmentsByProduct = async (req: Request, res: Response): Promise<void> => {
	try {
		if (!req.params.code || !req.params.ean) respond(req, res, 400, 'Cannot Get Assignments: Invalid Site Code or EAN Provided');
			...
			if (error && error.response && (error.response.status === 404 || error.response.status === 400)) respond(req, res, 400, 'Cannot Get Assignments: Invalid EAN Provided');
			else if (error) generate500(req, res, error);
		}); 
	} catch (error) {
		generate500(req, res, error);
...
export const deleteAssignment = async (req: Request, res: Response): Promise<void> => {
	try {
		if (!req.params.code || !req.params.aisle || !req.params.bay || !req.params.type || !req.params.ean) respond(req, res, 400, 'Cannot Unassign Product: Invalid Site Code, Aisle Number, Bay Number, Assignment Type or EAN Provided');
		else axios.get(`${config.base}/bay/${req.params.code}/${req.params.aisle}/${req.params.bay}`).then((response: AxiosResponse) => {
			const bay = response.data.data;
			axios.get(`${config.base}/product/${req.params.ean}`).then((response: AxiosResponse) => {
				...
				if (error && error.response && (error.response.status === 404 || error.response.status === 400)) respond(req, res, 400, 'Cannot Unassign Product: Invalid EAN Provided');
				else if (error) generate500(req, res, error);
			}); 
		}).catch((error: Error & { response: { status: number } } | null) => {
			if (error && error.response && (error.response.status === 404 || error.response.status === 400)) respond(req, res, 400, 'Cannot Unassign Product: Invalid Site Code, Aisle Number or Bay Number Provided');
			else if (error) generate500(req, res, error);
		}); 
	} catch (error) {
		generate500(req, res, error);
