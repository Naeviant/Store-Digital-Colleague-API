...
export const getModuleAtSite = async (req: Request, res: Response): Promise<void> => {
	try {
		if (!req.params.code || !req.params.discriminator) res.status(400).send(new ModuleInstanceResponse(400, 'Cannot Get Module at Site: Invalid Site Code or Discriminator')); 
			...
			axios.get(`${config.base}/module/${req.params.discriminator}`).then((response: AxiosResponse) => {
				...
				ModuleInstance.findOne({ module: module._id, site: site._id }, { __v: 0 })
					...
					.then((doc: IModuleInstance | null) => {
						if (!doc) res.status(404).send(new ModuleInstanceResponse(404, 'Cannot Get Module at Site: Module Not at Site'));
						else res.status(200).send(new ModuleInstanceResponse(200, 'Module at Site Retrieved Successfully', doc));
					}, (error: Error & { code: number } | null) => {
						if (error) generate500(req, res, error);
					});
			}).catch((error: Error & { response: { status: number } } | null) => {
		...
		if (!req.params.code || !req.params.discriminator) res.status(400).send(new ModuleInstanceResponse(400, 'Cannot Delete Module from Site: Invalid Site Code of Discriminator Body')); 
			...
			axios.get(`${config.base}/module/${req.params.discriminator}`).then((response: AxiosResponse) => {
				...
				ModuleInstance.deleteOne({ module: module._id, site: site._id }).then((doc: { deletedCount: number }) => {
					if (doc.deletedCount === 0) res.status(404).send(new ModuleInstanceResponse(404, 'Cannot Delete Module from Site: Module Not at Site'));
...
export const addModuleToBay = async (req: Request, res: Response): Promise<void> => {
	try {
		if (!req.params.code || !req.params.aisle || !req.params.bay) res.status(400).send(new ModuleInstanceResponse(400, 'Cannot Assign Module: Invalid Site Code, Aisle Number or Bay Number Provided'));
		else if (!req.body.discriminator) res.status(400).send(new ModuleInstanceResponse(400, 'Cannot Assign Module: Invalid Discriminator'));
			...
			axios.get(`${config.base}/module/${req.body.discriminator}`).then((response: AxiosResponse) => {
				...
				axios.get(`${config.base}/module/site/${req.params.code}/${req.params.aisle}/${req.params.bay}`).then((response: AxiosResponse) => {
					...
					if (bayModules.length >= bay.moduleLimit) res.status(400).send(new ModuleInstanceResponse(400, 'Cannot Assign Module: Bay is Full'));
						...
						if (docs.n === 0) res.status(400).send(new ModuleInstanceResponse(400, 'Cannot Assign Module: Invalid Site Code, Aisle Number, Bay Number or Discriminator Provided'));
						else res.status(200).send(new ModuleInstanceResponse(200, 'Module Assigned Successfully'));
					}, (error: Error & { code: number } | null) => {
						if (error) generate500(req, res, error);
					});
				}).catch((error: Error & { response: { status: number } } | null) => {
					if (error && error.response && (error.response.status === 404 || error.response.status === 400)) res.status(400).send(new ModuleInstanceResponse(400, 'Cannot Assign Module: Invalid Site Code, Aisle Number or Bay Number Provided'));
					else if (error) generate500(req, res, error);
				});
			}).catch((error: Error & { response: { status: number } } | null) => {
				if (error && error.response && (error.response.status === 404 || error.response.status === 400)) res.status(400).send(new ModuleInstanceResponse(400, 'Cannot Assign Module: Invalid Discriminator Provided'));
				else if (error) generate500(req, res, error);
			}); 
		}).catch((error: Error & { response: { status: number } } | null) => {
			if (error && error.response && (error.response.status === 404 || error.response.status === 400)) res.status(400).send(new ModuleInstanceResponse(400, 'Cannot Assign Module: Invalid Site Code, Aisle Number or Bay Number Provided'));
...
export const getModulesInBay = async (req: Request, res: Response): Promise<void> => {
	try {
		if (!req.params.code || !req.params.aisle || !req.params.bay) res.status(400).send(new ModuleInstanceResponse(400, 'Cannot Get Modules in Bay: Invalid Site Code, Aisle Number or Bay Number Provided'));
		else axios.get(`${config.base}/bay/${req.params.code}/${req.params.aisle}/${req.params.bay}`).then((response: AxiosResponse) => {
			const bay = response.data.data;
			ModuleInstance.find({ bay: bay._id }, { _id: 0, __v: 0 })
				...
				.populate({ path: 'bay', select: '-_id -__v', populate: { path: 'aisle', select: '-_id -__v', populate: { path: 'site', select: '-_id -__v' } } })
				.then((docs: IModuleInstance[] | null) => {
					res.status(200).send(new ModuleInstanceResponse(200, 'Modules in Bay Retrieved Successfully', docs));
				}, (error: Error & { code: number } | null) => {
					if (error) generate500(req, res, error);
				});
		}).catch((error: Error & { response: { status: number } } | null) => {
...
export const deleteModuleFromBay = async (req: Request, res: Response): Promise<void> => {
	try {
		if (!req.params.code || !req.params.aisle || !req.params.bay) res.status(400).send(new ModuleInstanceResponse(400, 'Cannot Unassign Module: Invalid Site Code, Aisle Number or Bay Number Provided'));
		else if (!req.params.discriminator) res.status(400).send(new ModuleInstanceResponse(400, 'Cannot Unassign Module: Invalid Discriminator'));
		else axios.get(`${config.base}/site/${req.params.code}`).then((response: AxiosResponse) => {
			const site = response.data.data;
			axios.get(`${config.base}/module/${req.params.discriminator}`).then((response: AxiosResponse) => {
				const module = response.data.data;
				ModuleInstance.updateOne({ site: site._id, module: module._id }, { '$set': { bay: null } }).then((docs: { n: number, nModified: number }) => {
					if (docs.n === 0) res.status(400).send(new ModuleInstanceResponse(400, 'Cannot Unassign Module: Invalid Site Code or Discriminator Provided'));
					else if (docs.nModified === 0) res.status(404).send(new ModuleInstanceResponse(404, 'Cannot Unassign Module: Module Not Assigned to Bay'));
					else res.status(200).send(new ModuleInstanceResponse(200, 'Module Unassigned Successfully'));
				}, (error: Error & { code: number } | null) => {
					if (error) generate500(req, res, error);
				});
			}).catch((error: Error & { response: { status: number } } | null) => {
				if (error && error.response && (error.response.status === 404 || error.response.status === 400)) res.status(400).send(new ModuleInstanceResponse(400, 'Cannot Unassign Module: Invalid Discriminator Provided'));
				else if (error) generate500(req, res, error);
			}); 
		}).catch((error: Error & { response: { status: number } } | null) => {
			if (error && error.response && (error.response.status === 404 || error.response.status === 400)) res.status(400).send(new ModuleInstanceResponse(400, 'Cannot Unassign Module: Invalid Site Code '));
			else if (error) generate500(req, res, error);
		}); 
	} catch (error) {
		generate500(req, res, error);
